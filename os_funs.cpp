...
void printSuperBlock() //打印超级块信息
{
	...
	printf("\tblock区开始位置：%d B\n", superblock->s_Block_StartAddr);			//block区开始位置
	printf("\n");
...
int balloc() //磁盘块分配函数
{
	...
	{ //剩余空闲块数为0//如果超级块中现实的剩余空闲块数为零，则返回-1
		printf("没有空闲块可以分配\n");
...
int ialloc() //分配i节点区函数，返回inode地址
{
	...
	{ 
		// L顺序的查找到空闲的inode返回并且进行分配的同时去更新超级块以及inode位图
		//顺序查找空闲的inode
		int i;
...
void rmall(int parinoAddr) //删除该节点下所有文件或目录
{
	...
	int cnt = cur.i_cnt;
	if(cnt<=2){
		bfree(cur.i_dirBlock[0]); //释放该节点的磁盘块
		ifree(parinoAddr);        //释放该节点的inode
		return ;
	}
...
{
	if(strlen(name)>=MAX_NAME_SIZE){    //判断是否出现目录名超长
		printf("超过最大目录名长度\n");
		return false;
	}

	if(strcmp(name,".")==0 || strcmp(name,"..")==0){  //判断是否为“.”或“..”等特殊项
		printf("错误操作\n");
		return 0;
	}
	...
	while(i<160){	                          //小于160
		...
		fread(&dirlist,sizeof(dirlist),1,fr); //将直接块内容读取文件
		...
		for (j = 0; j < 16; j++)
		{
			...
			if (strcmp(dirlist[j].itemName, name) == 0)
			{
				...
				{ //找到目录
					...
					rmall(dirlist[j].inodeAddr);           //删除该目录下的所有文件

					//删除该目录条目，写回磁盘
					strcpy(dirlist[j].itemName,"");        //将该目录条目的名字清空
					dirlist[j].inodeAddr = -1;             //清除目录inode的地址
					fseek(fw,parblockAddr,SEEK_SET);	   //将文件指针移动到inode的起始地址
					fwrite(&dirlist,sizeof(dirlist),1,fw); //写入文件//将直接块内容写入文件
					cur.i_cnt--;                           //减少目前已用inode的数量
					fseek(fw,parinoAddr,SEEK_SET);	       //将文件指针移动到inode的起始地址
					fwrite(&cur,sizeof(Inode),1,fw);       //写入文件
...
bool create(int parinoAddr, char name[], char buf[]) //创建文件函数，在该目录下创建文件，文件内容存在buf
{
	...
	{ //之前找到一个目录项了
		//取出之前那个空闲目录项对应的磁盘块
		fseek(fr,cur.i_dirBlock[posi],SEEK_SET);	//将文件指针移动到磁盘块的起始地址
		fread(dirlist,sizeof(dirlist),1,fr);		//将磁盘块内容读出
		fflush(fr);  //刷新读取缓存区
		...
		p.i_cnt = 1; //只有一个文件指向
				...
				for(k=0;k<len;k+=superblock->s_BLOCK_SIZE){	//最多10次，10个磁盘快，即最多5K//限定创建的文件的大小
					...
					if(curblockAddr==-1){  //磁盘块的地址不存在，表明分配失败
						printf("block分配失败\n");
						return false;
					}
...
{
	if (strlen(name) >= MAX_NAME_SIZE)//P需删除文件名超过字数限制
	{
		printf("超过最大目录名长度\n");
		return false;
	}
	...
	while(i<160){	//小于160，可以直接在直接块里找
		...
		//取出磁盘块
		int parblockAddr = cur.i_dirBlock[i/16];
		fseek(fr,parblockAddr,SEEK_SET);	 //将文件指针移动到磁盘块的起始地址
		fread(&dirlist,sizeof(dirlist),1,fr);//将磁盘块内容读出
...
void ls(int parinoAddr) //显示当前目录下的所有文件和文件夹。参数：当前目录的inode节点地址
{
	...
	//依次取出磁盘块
	int i = 0;
	while(i<cnt && i<160){//160项以内，可以直接在直接块里找
		DirItem dirlist[16] = {0};
		if(cur.i_dirBlock[i/16]==-1){
			i+=16;  //直接块为-1,跳过该块
			continue;
		...
		for (j = 0; j < 16 && i < cnt; j++)
		{
			...
			tm *ptr;	//存储时间
			ptr=gmtime(&tmp.i_mtime);  //记录修改时间

			//输出权限信息
			int t = 8;
			while(t>=0){
				if( ((tmp.i_mode>>t)&1)==1){
					if(t%3==2)	printf("r"); //只读
					if(t%3==1)	printf("w"); //可写
					if(t%3==0)	printf("x"); //完全控制
				}
				else{
					printf("-"); //无权限
				}
			...
			//其它
			printf("%d\t", tmp.i_cnt);          //P链接数																													
			printf("%s\t", tmp.i_uname);        //P用户数																												
			printf("%s\t", tmp.i_gname);        //P所属用户组																											
			printf("%d B\t", tmp.i_size);       //P文件大小																											
			printf("%d.%d.%d %02d:%02d:%02d  ", 1900 + ptr->tm_year, ptr->tm_mon + 1, ptr->tm_mday, (8 + ptr->tm_hour) % 24, ptr->tm_min, ptr->tm_sec); //上一次修改的时间
			printf("%s", dirlist[j].itemName);	//P文件名
			printf("\n");
			i++;
		}
	}
...
void cd(int parinoAddr, char name[]) //进入当前目录下的name目录
{
	...
	while(i<160){
		DirItem dirlist[16] = {0};//160项以内，可以直接在直接块里找
		if(cur.i_dirBlock[i/16]==-1){
			i+=16;//直接块为-1,跳过该块
			continue;
		}
				...
				if (((tmp.i_mode >> 9) & 1) == 1)
				{
					...
					else if (strcmp(dirlist[j].itemName, "..") == 0)
					{
						...
						for(k=strlen(Cur_Dir_Name);k>=0;k--)
							if(Cur_Dir_Name[k]=='/') //回到根目录，终止循环
								break;
						Cur_Dir_Name[k]='\0';//截断的文件结构能够知道前面的是什么
						if(strlen(Cur_Dir_Name)==0) //当前目录长度为零，则处于根目录
							Cur_Dir_Name[0]='/',Cur_Dir_Name[1]='\0';//根目录
					}
					else{
						if(Cur_Dir_Name[strlen(Cur_Dir_Name)-1]!='/') //如果没有打“/”//当前目录不是根目录
							strcat(Cur_Dir_Name,"/"); //加上“/”
						strcat(Cur_Dir_Name,dirlist[j].itemName); //将目录名连接起来//更换当前目录
					}
...
void gotoxy(HANDLE hOut, int x, int y) //移动光标到指定位置
{
      ...
      pos.X = x;            //横坐标
      pos.Y = y;            //纵坐标
      SetConsoleCursorPosition(hOut, pos); //设置控制台的光标位置
}
...
void vi(int parinoAddr, char name[], char buf[]) //模拟一个简单vi，输入文本，name为文件名
{
	...
	while(i<160){
		...
		fseek(fr,cur.i_dirBlock[dno],SEEK_SET);	//将文件指针移动到磁盘块的起始地址
		fread(dirlist,sizeof(dirlist),1,fr);    //将磁盘块的内容读出
		fflush(fr);
		...
		for (j = 0; j < 16; j++)
		{
			if (strcmp(dirlist[j].itemName, name) == 0)
			{
				//重名，取出inode，判断是否是文件
				fseek(fr,dirlist[j].inodeAddr,SEEK_SET);	//将文件指针移动到inode的起始地址
				fread(&fileInode,sizeof(Inode),1,fr);       //将inode内容读出
				if( ((fileInode.i_mode>>9)&1)==0 ){	//是文件且重名，打开这个文件，并编辑	
					fileInodeAddr = dirlist[j].inodeAddr;   //将文件的inode设置为取出的inode
					isExist = true;    //当前编辑的文件是存在的
					goto label;        // Dijkstra，提出“goto有害论” !
				}
			}
			i++;
		}
...
void cmd(char str[]) //处理输入的命令
{
	...
	{						// L判断是否显示inode位图
		printInodeBitmap(); // P显示inode位图
	}
	else if (strcmp(p1, "block") == 0)
	{								 // L判断是否显示block位图
		sscanf(str, "%s%s", p1, p2); //读取str的第二个字符串
		tmp = 0;
		if ('0' <= p2[0] && p2[0] <= '9')
		{
			tmp = atoi(p2);
			// for(i=0;p2[i];i++)
			// 	tmp = tmp*10+p2[i]-'0';
			printBlockBitmap(tmp);//P显示Block位图
		}
		else
			printBlockBitmap();
	}
