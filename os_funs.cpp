...
bool rmdir(int parinoAddr,char name[])	//目录删除函数
{
	...
	while(i<160){	//小于160
		...
		//取出磁盘块
		int parblockAddr = cur.i_dirBlock[i/16];
		fseek(fr,parblockAddr,SEEK_SET);	 //将文件指针移动到inode的起始地址
		fread(&dirlist,sizeof(dirlist),1,fr);//将inode内容读出
			...
			if(strcmp(dirlist[j].itemName,name)==0){
				...
				if( ( (tmp.i_mode>>9) & 1 ) == 1 ){
					...
					if( strcmp(dirlist[j].itemName,".")==0){
						...
						if(Cur_Dir_Name[strlen(Cur_Dir_Name)-1]!='/') //如果没有打“/”
							strcat(Cur_Dir_Name,"/"); //加上“/”
						strcat(Cur_Dir_Name,dirlist[j].itemName); //将目录名连接起来
					}
